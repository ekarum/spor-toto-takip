...
function getExactDegreeGroups(results=state.results){
  const groups={15:[],14:[],13:[],12:[],below:[]};
  state.columns.forEach((col,index)=>{
    let wrong=0;
    for(let i=0;i<15;i++)if(results[i]&&col[i]!==results[i])wrong++;
    const row={index,column:col,wrong,maxScore:15-wrong};
    if(wrong===0)groups[15].push(row);
    else if(wrong===1)groups[14].push(row);
    else if(wrong===2)groups[13].push(row);
    else if(wrong===3)groups[12].push(row);
    else groups.below.push(row);
  });
  return groups;
}
function buildExactGroupComment(degree,count,total,entered,remaining){
  if(!count){
    if(degree===15)return entered?'15 doğru şansını koruyan kolon kalmadı.':'Yüklenen kolon bulunmuyor.';
    return `Şu anda en yüksek ${degree} doğruya ulaşabilecek kolon bulunmuyor.`;
  }
  const rate=total?count/total*100:0;
  if(degree===15){
    if(!remaining)return `${count.toLocaleString('tr-TR')} kolon haftayı tam 15 doğruyla tamamladı.`;
    return `${count.toLocaleString('tr-TR')} kolon hâlâ hatasız ilerliyor ve 15 doğru şansını koruyor.`;
  }
  if(degree===14)return `${count.toLocaleString('tr-TR')} kolonun bir yanlışı var; 15 ihtimali bitti ancak tam 14 şansı devam ediyor.`;
  if(degree===13)return `${count.toLocaleString('tr-TR')} kolonun iki yanlışı var ve ulaşabileceği en yüksek derece tam 13.`;
  return `${count.toLocaleString('tr-TR')} kolonun üç yanlışı var ve ulaşabileceği en yüksek derece tam 12.`;
}
function buildKarumNarrative(data){
  const {entered,remaining,groups,total,critical}=data;
  const c15=groups[15].length,c14=groups[14].length,c13=groups[13].length,c12=groups[12].length,below=groups.below.length;
  if(!remaining){
    const best=c15?15:c14?14:c13?13:c12?12:'12 altı';
    return `Hafta tamamlandı. En yüksek derece ${best}. Dağılım: ${c15} tam 15, ${c14} tam 14, ${c13} tam 13 ve ${c12} tam 12 kolon.`;
  }
  let text=`${entered} maç sonuçlandı, ${remaining} maç kaldı. ${c15.toLocaleString('tr-TR')} kolon 15’e, ${c14.toLocaleString('tr-TR')} kolon tam 14’e, ${c13.toLocaleString('tr-TR')} kolon tam 13’e ve ${c12.toLocaleString('tr-TR')} kolon tam 12’ye kaldı.`;
  if(below)text+=` ${below.toLocaleString('tr-TR')} kolon artık 12’nin üzerine çıkamaz.`;
  const top=critical[0];
  if(top)text+=` 15 şansı açısından en kritik karşılaşma ${top.index+1}. maç: ${state.matchNames[top.index]}.`;
  if(!c15&&c14)text+=` 15 ihtimali sona erdi; sistemin en yüksek canlı hedefi artık 14.`;
  return text;
}
function renderKarumZeka(){
  ...
  empty.classList.add('hidden');content.classList.remove('hidden');
  const groups=getExactDegreeGroups(),tracked=groups[15].length+groups[14].length+groups[13].length+groups[12].length;
  $('kzPlayed').textContent=entered;$('kzRemaining').textContent=remaining;$('kzTracked').textContent=tracked.toLocaleString('tr-TR');$('kzBelow12').textContent=groups.below.length.toLocaleString('tr-TR');$('kzWeekName').textContent=state.weekName||'Güncel Hafta';
  const token=++karumZekaJobToken,results=state.results.slice();
  setTimeout(()=>{
    if(token!==karumZekaJobToken)return;
    const pending=results.map((v,i)=>!v?i:-1).filter(i=>i>=0);
    const critical=pending.map(index=>{
      const outcomes=['1','X','2'].map(pick=>{const next=results.slice();next[index]=pick;const nextGroups=getExactDegreeGroups(next);return{pick,count15:nextGroups[15].length,count14:nextGroups[14].length,count13:nextGroups[13].length,count12:nextGroups[12].length}});
      const values=outcomes.map(o=>o.count15);return{index,outcomes,spread:Math.max(...values)-Math.min(...values),min:Math.min(...values)};
    }).sort((a,b)=>b.spread-a.spread||a.min-b.min).slice(0,5);
    karumZekaData={total,entered,remaining,groups,critical};
    $('kzNarrative').textContent=buildKarumNarrative(karumZekaData);
    const labels={15:'15’e Kalan Kuponlar',14:'Tam 14’e Kalan Kuponlar',13:'Tam 13’e Kalan Kuponlar',12:'Tam 12’ye Kalan Kuponlar'};
    const tones={15:'degree-15',14:'degree-14',13:'degree-13',12:'degree-12'};
    $('kzExactGrid').innerHTML=[15,14,13,12].map(k=>{
      const count=groups[k].length,rate=total?count/total*100:0;
      return `<article class="kz-exact-card ${tones[k]}"><div class="kz-exact-top"><span>${k}</span><div><small>${labels[k]}</small><strong>${count.toLocaleString('tr-TR')} <em>kolon</em></strong></div></div><div class="kz-exact-bar"><i style="width:${Math.min(100,rate).toFixed(1)}%"></i></div><p>${buildExactGroupComment(k,count,total,entered,remaining)}</p><footer>Toplam kolonların %${rate.toFixed(1).replace('.',',')}</footer></article>`;
    }).join('');
    $('kzCriticalList').innerHTML=critical.length?critical.map((m,rank)=>`<article class="kz-critical-card"><div class="kz-critical-head"><span>${rank+1}</span><div><strong>${m.index+1}. Maç</strong><small>${escapeHtml(state.matchNames[m.index]||'')}</small></div></div><div class="kz-impact-grid">${m.outcomes.map(o=>`<div class="${o.count15===0?'danger':''}"><b>${o.pick}</b><strong>${o.count15.toLocaleString('tr-TR')}</strong><small>15’e kalan</small><em>${o.count14.toLocaleString('tr-TR')} tam 14</em></div>`).join('')}</div></article>`).join(''):'<div class="kz-empty-mini">Bekleyen maç kalmadı.</div>';
  },20);
}
