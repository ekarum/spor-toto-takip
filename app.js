...
function extractMatchData(rows){
  ...
  let matchNames=null,results=Array(15).fill('');
  ...
  return {matchNames:matchNames||Array.from({length:15},(_,i)=>`Maç ${i+1}`),results};
}
function parseWorkbook(wb,fileName){
  ...
  const {matchNames,results}=extractMatchData(panelRows);
